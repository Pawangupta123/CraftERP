...
-- Carton/packing sizes (cm). skus.cbm is computed from these = Σ(L×W×H/1e6 ÷ pcs_per_carton).
create table public.carton_components (
  id             uuid primary key default gen_random_uuid(),
  sku_id         uuid not null references public.skus(id) on delete cascade,
  description    text, length numeric, width numeric, height numeric, pcs_per_carton numeric,
  position       int default 0
);
